...
import { defined, Camera, Rectangle, Cartographic, Math as CesiumMath } from "cesium/Cesium"
...
ResetViewNavigationControl.prototype.resetView = function () {
  ...
  if (defined(this.terria.trackedEntity)) {
    ...
    } else if (this.terria.options.defaultResetLocaltion) {
      camera.setView({
        destination: Cesium.Cartesian3.fromDegrees(Number(this.terria.options.defaultResetLocaltion[0]), Number(this.terria.options.defaultResetLocaltion[1]), Number(this.terria.options.defaultResetLocaltion[2])),
        orientation: {
          heading: Number(this.terria.options.defaultResetLocaltion[3]),
          pitch: Number(this.terria.options.defaultResetLocaltion[4]),
          roll: 0.0,
        },
      });
